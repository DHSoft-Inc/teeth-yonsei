...
public class EditPopupRenderCommand implements MVCRenderCommand {
	  @Override
	   public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
	      ...
	      List<TreatmentHistory> EditHistoryList = new ArrayList<>(); // 寃곌?��?��?���?? �쟾�넚�맆 TreatmentHistory List 
	      List<Long> IDList = new ArrayList<>(); // �뙆�떛�맂 ID �꽔�뒗 List
	      
	      // TreatmentIDList濡� 諛쏆�?? IDList?���?? �뙆�떛
	      if (TreatmentIDList != null && !TreatmentIDList.isEmpty()) {
	          // �돹�몴濡� ?��꾨━
	          String[] tokens = TreatmentIDList.split(",");
	          for (String token : tokens) {
	              // �샊�떆 ?�듬갚�?�� �꽎�뿬�엳�쓣 寃쎌?��?���?? ��?��꾪빐 trim()
	              token = token.trim();
	              if (!token.isEmpty()) {
	                  // Long�쑝濡� 蹂��솚 �썑 ?��?�뒪�듃�뿉 ?��붽�
	      ...
	      // �뙆�떛�맂 ID?���?? �넗��濡� EditHistory List�뿉 TreatmentHistory ?��붽�
	      for (Long ID : IDList)
	      {
	         try {
